...
| 01 Security / production baseline | IN PROGRESS | Next.js patched to 16.3.6 with lockfile regenerated by GitHub Actions; typecheck/test scripts, env isolation, contact false-success fix and CI are in place. Final CI on patched state pending. |
...
- [x] Upgrade `next` and `eslint-config-next` to 16.3.6 and regenerate the lockfile together via GitHub Actions.
...
- Next.js 16.3.6 is the verified current security patch as of 2026-09-25; the manifest and lockfile were regenerated together by GitHub Actions.
